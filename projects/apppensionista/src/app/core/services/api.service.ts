...
@Injectable({
  ...
  export class ApiService {
    ...
    private formatErrors(operation: any) {
      ...
      public async getAsync<T>(path: string): Promise<any> {
        // this.url = `${environment.apiServiceSeguridadUrl}${path}`;
        this.url = `${path}`;
        const result =  await this.http.get<T>(this.url).toPromise();//.catch(catchError(this.formatErrors('get')));         
        return result;
      }
      public async postAsync<T>(path: string, body: Object = {}): Promise<any> {
        // this.url = `${environment.apiServiceSeguridadUrl}${path}`;
        this.url = `${path}`;
